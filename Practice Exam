...
• A Jinja2 template file `index.html.j2` is used to create the file `/var/www/html/index.html` with the following content:
   ...
   ```
This webserver: HOSTNAME is being accessed at: IPADDRESS (where HOSTNAME and IPADDRESS are from the actual host in the webservers group)
   ```
...
• Ensure the proper SELInux file context is applied.
 - After running the playbook, it should be possible run the command curl http://<webserver IP>/webadmin/hello.html and see the content of the file
